...
public class IO {
	...
	/*
	 * static int totalKeyCount= 1; ArrayList<Writer> writerCollection = new
	 * ArrayList<>(); ArrayList<Reader> readerCollection = new ArrayList<>();
	 * 
	 * Map <String, File> input = new HashMap<>() ; Map <String, File> output =
	 * new HashMap<>();
	 * 
	 */
	...
	public void handle(InputDataPacket dp) {
		...
		// input.put("theKey", theFile); //bullshit code but shows order. if
		// saved in Maps
	}
	...
	public void moveToSend(File f, String deviceID) {
		...
		f.renameTo(new File("C:\\Users\\Johan\\IoT\\git\\Communication-Application\\files\\" + deviceID + "\\sent\\"
				+ f.getName()));
	}
	 ...
	 * 
	 * public void createWriter(){ Writer writer = new Writer();
	 * writerCollection.add(writer); } public void createReader(){ Reader reader
	 * = new Reader(); readerCollection.add(reader); }
	 */
}
